...
if [ "$color_prompt" = yes ]; then
  if [ -n "$SSH_CLIENT" ]; then
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;31m\]$(whoami)@$(uname -n)\[\033[00m\] \[\033[01;36m\]\w\[\033[00m\]\$ '
  else
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]$(whoami)@$(uname -n)\[\033[00m\] \[\033[01;36m\]\w\[\033[00m\]\$ '
  fi
else
...
#   sleep 10; alert
alias alert='notify-send --urgency=low --expire-time=1000 -i "$([ $? = 0 ] && echo terminal || echo error)" "$(history|tail -n1|sed -e '\''s/^\s*[0-9]\+\s*//;s/[;&|]\s*alert$//'\'')"'
...
# Set LANG to remove curley quotes and all that crap
